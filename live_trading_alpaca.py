...
class AlpacaLiveTrader:
    ...
    def __init__(self):
        self.broker = AlpacaBroker(api_key=ALPACA_API_KEY, secret_key=ALPACA_SECRET_KEY, paper=True)
    ...
    def run_once(self, current_price: float) -> None:
        ...
        if confidence > 0.50:
            if position is None:
                ...
                if signal_side == "BUY":
                    try:
                        self.broker.place_order(self.symbol, "buy", self.position_size)
                        self.trade_count += 1
                ...
                try:
                    ...
                    self.trade_count += 1
                    logger.info(
                        f"🔴 CLOSE LONG {self.symbol} | P&L: ${position['unrealized_pl']:.2f}"
                    )
                except Exception as e:
