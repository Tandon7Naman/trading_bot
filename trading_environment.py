...
class GoldTradingEnv(gym.Env):
    ...
    def __init__(
        self, df, lstm_model, initial_capital=100000, transaction_cost=0.0005, slippage=10
    ):
        super().__init__()
        ...
        print("   Observation dim: 16")
        print("   Action space: [HOLD, BUY, SELL]")
        ...
        if len(self.trades) > 0:
            recent_trades = len(
                [
                    ...
                    if t.get("exit_step") is not None and t["exit_step"] > self.current_step - 10
                ]
            )
            if recent_trades > 3:
                reward -= 0.1
    ...
    def _execute_sell(self, price):
        ...
        # Update last trade
        self.trades[-1].update({"exit_step": self.current_step, "exit_price": proceeds, "pnl": pnl})
    ...
    def _get_observation(self):
        ...
        # 16: LSTM signal
        lstm_window = self.df.loc[
            self.current_step - 60 : self.current_step - 1, ["open", "high", "low", "close"]
        ].values
        lstm_signal = self.lstm_model.predict(lstm_window) if len(lstm_window) == 60 else 0.5
    ...
    def get_metrics(self):
        ...
        # Calculate returns
        returns = np.array([t["pnl"] / self.initial_capital for t in closed_trades])
