...
class RiskManager:
    ...
    @staticmethod
    def calculate_lot_size(equity, entry_price, sl_price, symbol, confidence_score=1.0, current_portfolio_risk_usd=0.0):
        """
        ...
        if available_risk_budget <= 0:
            print(f"   🛡️ RISK VETO: Portfolio Overloaded! (Locked Risk: ${current_portfolio_risk_usd:.2f})")
            return 0.0
        ...
        print(f"   ⚖️  RISK MATH: Target Risk ${final_risk_usd:.2f} ({target_risk_pct*100:.1f}%) | Portfolio Budget: ${available_risk_budget:.2f} -> Size {lots}")
        return round(lots, 2)
