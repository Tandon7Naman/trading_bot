...
with st.sidebar:
    ...
    st.text("Queue: XAUUSD")
    st.text("Latency: < 50ms")
...
kpi4.metric(
    "Active Margin", f"${position['margin_locked']:,.2f}" if position != "FLAT" else "$0.00"
)

st.markdown("---")

# 2. TABS LAYOUT
tab_live, tab_perf, tab_raw = st.tabs(
    ["📈 Live Market", "📊 Performance Analysis", "💾 System State"]
)
...
with tab_live:
    ...
    with col_chart:
        if not df.empty:
            ...
            if position != "FLAT":
                ...
                fig.add_hline(
                    y=sl, line_dash="dot", line_color="red", annotation_text="SL", row=1, col=1
                )
                fig.add_hline(
                    y=tp, line_dash="dot", line_color="green", annotation_text="TP", row=1, col=1
                )
    ...
    with col_orders:
        ...
        if pending:
            ...
            st.dataframe(
                pending_df[["type", "limit_price", "qty"]], hide_index=True, width="stretch"
            )
        else:
...
st.markdown("---")
st.caption(
    f"System Time (UTC): {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')} | Protocol 7.2 Active"
)
