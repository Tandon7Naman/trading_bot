...
def train_ppo_agent():
    ...
    episode_rewards = agent.train(
        total_timesteps=5000,  # Change to 100000 for full training
        save_freq=1000,
    )
...
def train_ppo_agent():
    ...
    if metrics:
        ...
        # Check if meets Blueprint targets
        print("\n✅ Blueprint Validation:")
        print(f"   Sharpe > 1.2: {'✅ PASS' if metrics['sharpe_ratio'] > 1.2 else '❌ FAIL'}")
        print(f"   Max DD < -10%: {'✅ PASS' if metrics['max_drawdown'] > -10 else '❌ FAIL'}")
        print(f"   Win Rate > 45%: {'✅ PASS' if metrics['win_rate'] > 45 else '❌ FAIL'}")
