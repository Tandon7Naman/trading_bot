...
class LSTMSignalPredictor:
    ...
    def predict_signal(self, X):
        ...
        return {"signal": signal, "probability": float(prob), "confidence": float(confidence)}
...
class MockLSTMSignalPredictor:
    ...
    def predict_signal(self, X):
        prob = np.random.rand()
        signal = "BUY" if prob > 0.5 else "SELL"
        return {"signal": signal, "probability": float(prob), "confidence": max(prob, 1 - prob)}

    def save(self, path):
        print("⚠️  Mock model not saved")

    def load(self, path):
        print("⚠️  Mock model loaded")
