...
class PaperTradingEngine:
    ...
    def close_position(
        self, symbol: str, exit_price: float, entry_price: float = 0, position_size: int = 1
    ) -> dict[str, Any]:
        """
        ...
        logger.info(
            f"Position closed: {symbol} @ ₹{exit_price:,.0f} | P&L: ₹{pnl:,.0f} ({pnl_percent:.2f}%)"
        )
    ...
    def update_balance(self, pnl: float):
        ...
        self.drawdown = (
            ((self.max_equity - self.equity) / self.max_equity) * 100 if self.max_equity > 0 else 0
        )
